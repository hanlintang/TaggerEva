...
def compare(corpus, path='./new_data/et_out'):
    ...
    #     print(filename)
    #     et_result = open(os.path.join(path, filename), 'r').readlines()
    ...
    poses = true_results
    # et_result = pd.read_csv('./add_et_retrain.csv')['et'].values.tolist()
    # et_result = [line.strip().split(' ') for line in et_result]
    # data = pd.read_csv('./new_data/method/test/pos_nltk_hadoop_method.csv')
    # poses = data['pos_tag'].values.tolist()[:1000]
    # poses = pd.read_csv('./add_et_retrain.csv')['pos'].values.tolist()
    # poses = [line.strip().split(' ') for line in poses]
...
if __name__ == '__main__':
    # config = Config()
    # corpus = Corpus(config.data_path, config.mode[0])
    # vocab = Vocab(corpus)
    ...
    # create_et_data(config, corpus.test_input, corpus.test_tags)
    # create_data(corpus.train_input, corpus.train_tags)

    # compare_st()
    config = Config()

    # et evaluation
    taggers = ['OUT', 'SWUM', 'POSSE', 'STANFORD']
    out_files = ['method', 'args', 'class', 'all']
    path = './ensemble_output_IDData/'
    ...
    for out_file in out_files:
        ...
        if out_file == 'method':
            ...
            et_mode = 'ALL'
        file_path = path + "et_out_"+et_mode + '_new.csv'
        golden_path = '../evaluation_results/evaluation_' + out_file + '.csv'
        for tagger in taggers:
            swum_posse_results(file_path, golden_path, tagger, with_unk=config.with_unk)
